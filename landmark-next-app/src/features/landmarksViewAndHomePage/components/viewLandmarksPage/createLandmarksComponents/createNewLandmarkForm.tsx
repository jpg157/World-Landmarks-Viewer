...
type CreateNewLandmarkFormProps = {
  onClose: () => void;
}

const CreateNewLandmarkForm = ({ 
  onClose 
}: CreateNewLandmarkFormProps) => {
  ...
  async function handleFormSubmit() { // uncontrolled form input data unused
    ...
    try {
      const newLandmarkExclImage = await createLandmark(landmarkData);
      ...
      await setAntiforgeryTokenHeaders();
      
      await uploadLandmarkImageFile({
...
export default CreateNewLandmarkForm
